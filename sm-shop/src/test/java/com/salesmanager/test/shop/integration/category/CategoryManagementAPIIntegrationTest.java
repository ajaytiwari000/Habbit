...
public class CategoryManagementAPIIntegrationTest extends ServicesTestSupport {
  ...
  public void getCategory() throws Exception {
    ...
    if (response.getStatusCode() != HttpStatus.OK) {
      ...
      final List<ReadableCategorys> categories = response.getBody().getCategories();
  ...
  public void postCategory() throws Exception {
    ...
    ResponseEntity response =
        testRestTemplate.postForEntity(
            "/api/v1/private/category", entity, PersistablesCategory.class);
    PersistablesCategory cat = (PersistablesCategory) response.getBody();
  ...
  public void putCategory() throws Exception {
    ...
    final ResponseEntity response =
        testRestTemplate.postForEntity(
            "/api/v1/private/category", entity, PersistablesCategory.class);
    final PersistablesCategory cat = (PersistablesCategory) response.getBody();
    assertThat(response.getStatusCode(), is(CREATED));
    assertNotNull(cat.getId());

    HttpEntity<String> httpEntity = new HttpEntity<>(getHeader());

    final ResponseEntity<ReadableCategorys> readableQuery =
        testRestTemplate.exchange(
            String.format("/api/v1/categoryList/" + cat.getId()),
            HttpMethod.GET,
            httpEntity,
            ReadableCategorys.class);

    assertThat(readableQuery.getStatusCode(), is(OK));

    ReadableCategorys readableCategorys = readableQuery.getBody();
    newCategory = new PersistablesCategory();
    newCategory.setCode("angular");
    newCategory.setVisible(true);
    newCategory.setDepth(4);
    ...
    HttpEntity<PersistablesCategory> requestUpdate = new HttpEntity<>(newCategory, getHeader());

    ResponseEntity resp =
        testRestTemplate.exchange(
            "/api/v1/private/category/" + cat.getId(), HttpMethod.PUT, requestUpdate, Void.class);
  ...
  public void postComplexCategory() throws Exception {
    ...
    /** Dining room * */
    final PersistablesCategory dining = new PersistablesCategory();
    ...
    /** armoire * */
    final PersistablesCategory armoire = new PersistablesCategory();
    ...
    /** benches * */
    final PersistablesCategory bench = new PersistablesCategory();
    bench.setCode("bench");
    ...
    /** Living room * */
    final PersistablesCategory living = new PersistablesCategory();
    ...
    /** lounge * */
    final PersistablesCategory lounge = new PersistablesCategory();
  ...
  public void deleteCategory() throws Exception {
    ...
    testRestTemplate.exchange(
        "/services/DEFAULT/category/100", HttpMethod.DELETE, httpEntity, Category.class);
  }
  ...
  public void manufacturerForItemsInCategory() throws Exception {
    ...
    ResponseEntity response =
        testRestTemplate.postForEntity(
            "/api/v1/private/manufacturer", entity, PersistableManufacturer.class);
    assertThat(response.getStatusCode(), is(CREATED));
    ...
    // create category
    PersistablesCategory category = super.category("TEST");
    ...
    final PersistablesCategory persistable = (PersistablesCategory) categoryResponse.getBody();

    Long id = persistable.getId();

    // create first item

    PersistablesProduct product1 = super.product("PRODUCT1");
    ...
    PersistablesProduct product2 = super.product("PRODUCT2");
    ...
    @SuppressWarnings("rawtypes")
    ResponseEntity<List> manufacturers =
        testRestTemplate.exchange(
            String.format("/api/v1/category/" + id + "/manufacturers"),
            HttpMethod.GET,
            entity,
            List.class);
