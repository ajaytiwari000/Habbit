...
public class OrderControler {
  ...
  @Inject EmailsService emailsService;

  @Inject private EmailUtils emailUtils;

  @Inject OrderProductDownloadService orderProdctDownloadService;

  private static final String ORDER_STATUS_TMPL = "email_template_order_status.ftl";

  @PreAuthorize("hasRole('ORDER')")
  @RequestMapping(value = "/admin/orders/editOrder.html", method = RequestMethod.GET)
  public String displayOrderEdit(
  ...
  private String displayOrder(
      Long orderId, Model model, HttpServletRequest request, HttpServletResponse response)
      throws Exception {

    // display menu
    setMenu(model, request);

    com.salesmanager.shop.admin.model.orders.Order order =
        new com.salesmanager.shop.admin.model.orders.Order();
    ...
    if (orderId != null && orderId != 0) { // edit mode
      ...
      List<OrderProductDownload> orderProductDownloads =
          orderProdctDownloadService.getByOrderId(order.getId());
  ...
  public String saveOrder(
    ...
    if (!StringUtils.isBlank(entityOrder.getDatePurchased())) {
      try {
        ...
        ObjectError error =
            new ObjectError("datePurchased", messages.getMessage("message.invalid.date", locale));
    ...
    if (!StringUtils.isBlank(entityOrder.getOrder().getCustomerEmailAddress())) {
      java.util.regex.Matcher matcher =
          pattern.matcher(entityOrder.getOrder().getCustomerEmailAddress());
    ...
    //      ObjectError error =
    //          new ObjectError(
    //              "billingFirstName", messages.getMessage("NotEmpty.order.billingFirstName",
    // locale));
    //      result.addError(error);
    //    }
    //
    ...
    //      ObjectError error =
    //          new ObjectError(
    //              "billingLastName", messages.getMessage("NotEmpty.order.billingLastName",
    // locale));
    //      result.addError(error);
    //    }
    //
    ...
    //      ObjectError error =
    //          new ObjectError(
    //              "billingAddress", messages.getMessage("NotEmpty.order.billingStreetAddress",
    // locale));
    //      result.addError(error);
    //    }
    //
    ...
    //      ObjectError error =
    //          new ObjectError("billingCity", messages.getMessage("NotEmpty.order.billingCity",
    // locale));
    //      result.addError(error);
    //    }
    //
    ...
    //        ObjectError error =
    //            new ObjectError(
    //                "billingState", messages.getMessage("NotEmpty.order.billingState", locale));
    //        result.addError(error);
    //      }
    //    }
    //
    ...
    //      ObjectError error =
    //          new ObjectError(
    //              "billingPostalCode", messages.getMessage("NotEmpty.order.billingPostCode",
    // locale));
    //      result.addError(error);
    //    }

    com.salesmanager.core.model.order.Order newOrder =
        orderService.getById(entityOrder.getOrder().getId());
    ...
    //
    // countryService.getByCode(entityOrder.getOrder().getDelivery().getCountry().getIsoCode());
    //    Country billingCountry =
    //
    // countryService.getByCode(entityOrder.getOrder().getBilling().getCountry().getIsoCode());
    //    Zone billingZone = null;
    //    Zone deliveryZone = null;
    //    if (entityOrder.getOrder().getBilling().getZone() != null) {
    //      billingZone =
    // zoneService.getByCode(entityOrder.getOrder().getBilling().getZone().getCode());
    //    }
    //
    //    if (entityOrder.getOrder().getDelivery().getZone() != null) {
    //      deliveryZone =
    //          zoneService.getByCode(entityOrder.getOrder().getDelivery().getZone().getCode());
    //    }
    ...
    List<OrderProductDownload> orderProductDownloads =
        orderProdctDownloadService.getByOrderId(newOrder.getId());
    if (CollectionUtils.isNotEmpty(orderProductDownloads)) {
      model.addAttribute("downloads", orderProductDownloads);
    }
    ...
    if (StringUtils.isBlank(entityOrder.getOrderHistoryComment())) {
      ...
      try {
        ...
        //        customerName
        //            .append(newOrder.getBilling().getFirstName())
        //            .append(" ")
        ...
        templateTokens.put(
            EmailConstants.EMAIL_TEXT_ORDER_NUMBER,
            messages.getMessage(
                "email.order.confirmation",
                new String[] {String.valueOf(newOrder.getId())},
                customerLocale));
        templateTokens.put(
            EmailConstants.EMAIL_TEXT_DATE_ORDERED,
            messages.getMessage(
                "email.order.ordered",
                new String[] {entityOrder.getDatePurchased()},
                customerLocale));
        templateTokens.put(
            EmailConstants.EMAIL_TEXT_STATUS_COMMENTS,
            messages.getMessage(
                "email.order.comments",
                new String[] {entityOrder.getOrderHistoryComment()},
                customerLocale));
        templateTokens.put(
            EmailConstants.EMAIL_TEXT_DATE_UPDATED,
            messages.getMessage(
                "email.order.updated",
                new String[] {DateUtil.formatDate(new Date())},
                customerLocale));
