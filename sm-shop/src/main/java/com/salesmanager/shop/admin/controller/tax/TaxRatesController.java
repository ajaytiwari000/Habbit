...
public class TaxRatesController {
  ...
  private static final char DECIMALCOUNT = '3';
  ...
  @RequestMapping(
      value = {"/admin/tax/taxrates/list.html"},
      method = RequestMethod.GET)
  public String displayTaxRates(
  ...
  public @ResponseBody ResponseEntity<String> pageTaxRates(
  ...
  public String saveTaxRate(
      @Valid @ModelAttribute("taxRate") TaxRate taxRate,
  ...
  private void validateTaxRate(
    ...
    if (StringUtils.isBlank(taxRate.getRateText())) {
      FieldError error =
          new FieldError(
              "taxRate", "rateText", messages.getMessage("NotEmpty.taxRate.rateText", locale));
      result.addError(error);
    }

    // if(taxRate.isPiggyback() && taxRate.getParent()==null) {
    // FieldError error = new
    // FieldError("taxRate","piggyback",messages.getMessage("NotNull.taxRate.parent", locale));
    // result.addError(error);
    // }
    ...
    try {
      ...
      FieldError error =
          new FieldError(
              "taxRate", "rateText", messages.getMessage("message.invalid.rate", locale));
      result.addError(error);
    }
    ...
    if (tr != null) {
      if (tr.getId().longValue() != taxRate.getId().longValue()) {
        FieldError error =
            new FieldError(
                "taxRate", "code", messages.getMessage("NotEmpty.taxRate.unique.code", locale));
        result.addError(error);
      }
    }
  ...
  public @ResponseBody ResponseEntity<String> removeTaxRate(
    ...
    try {
      ...
      /** In order to remove a User the logged in must be STORE_ADMIN or SUPER_USER */
      if (taxRateId == null) {
