...
public class CustomerOptionsSetController {
  ...
  @Inject private CustomerOptionSetService customerOptionSetService;
  ...
  public String displayOptions(
    ...
    // get values
    List<CustomerOptionValue> optionsValues =
        customerOptionValueService.listByStore(store, language);
  ...
  public String saveOptionSet(
      @Valid @ModelAttribute("optionSet") CustomerOptionSet optionSet,
    ...
    // CustomerOptionValue optionValue =
    // customerOptionValueService.getById(optionSet.getPk().getCustomerOptionValue().getId());
    CustomerOptionValue optionValue =
        customerOptionValueService.getById(optionSet.getCustomerOptionValue().getId());
    ...
    if (optionsSet != null && optionsSet.size() > 0) {
      ...
      for (CustomerOptionSet optSet : optionsSet) {
        ...
        if (opt.getId().longValue() == optionSet.getCustomerOption().getId().longValue()
            // && optValue.getId().longValue() ==
            // optionSet.getPk().getCustomerOptionValue().getId().longValue()) {
            && optValue.getId().longValue()
                == optionSet.getCustomerOptionValue().getId().longValue()) {
          model.addAttribute(
              "errorMessageAssociation",
  ...
  public @ResponseBody ResponseEntity<String> pageOptionsSet(
    ...
    try {
      ...
      if (optionSet != null && optionSet.size() > 0) {
        ...
        for (CustomerOptionSet optSet : optionSet) {
          ...
          CustomerOptionValueDescription valueDescription =
              customerOptionValue.getDescriptionsSettoList().get(0);
  ...
  public @ResponseBody ResponseEntity<String> deleteOptionSet(
    ...
    try {
      ...
      // if(entity==null ||
      // entity.getPk().getCustomerOption().getMerchantStore().getId().intValue()!=store.getId().intValue()) {
      if (entity == null
          || entity.getCustomerOption().getMerchantStore().getId().intValue()
  ...
  public @ResponseBody ResponseEntity<String> updateOrder(
      HttpServletRequest request, HttpServletResponse response) {
