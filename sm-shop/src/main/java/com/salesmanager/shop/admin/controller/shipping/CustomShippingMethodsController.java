...
public class CustomShippingMethodsController {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(CustomShippingMethodsController.class);

  public static final String WEIGHT_BASED_SHIPPING_METHOD = "weightBased";

  @Inject private ShippingService shippingService;

  @Inject private CountryService countryService;

  @Inject private ProductPriceUtils priceUtil;
  ...
  public String getWeightBasedShippingMethod(
  ...
  public String addCustomRegion(
    ...
    CustomShippingQuotesConfiguration customConfiguration =
        (CustomShippingQuotesConfiguration)
            shippingService.getCustomShippingConfiguration(WEIGHT_BASED_SHIPPING_METHOD, store);
    ...
    if (StringUtils.isBlank(region)) {
      ...
      ObjectError error =
          new ObjectError("region", messages.getMessage("message.region.null", locale));
    ...
    for (CustomShippingQuotesRegion customRegion : regions) {
      if (customRegion.getCustomRegionName().equals(region)) {
        model.addAttribute("errorMessage", messages.getMessage("message.region.null", locale));
        ObjectError error =
            new ObjectError("region", messages.getMessage("message.region.exists", locale));
        result.addError(error);
        break;
      }
    }
    ...
    shippingService.saveCustomShippingConfiguration(
        this.WEIGHT_BASED_SHIPPING_METHOD, customConfiguration, store);
  ...
  public String addCountryToCustomRegion(
      @ModelAttribute("customRegion") CustomShippingQuotesRegion customRegion,
      BindingResult result,
      Model model,
      HttpServletRequest request,
      HttpServletResponse response,
    ...
    if (StringUtils.isBlank(customRegion.getCustomRegionName())) {
      model.addAttribute(
          "errorMessageAssociation", messages.getMessage("message.region.null", locale));
      ObjectError error =
          new ObjectError("region", messages.getMessage("message.region.exists", locale));
      result.addError(error);
    }
        ...
        if (countries != null) {
          for (String countryCode : countries) {
            if (countryCode.equals(customRegion.getCountries().get(0))) {
              model.addAttribute(
                  "errorMessageAssociation", messages.getMessage("message.region.exists", locale));
              ObjectError error =
                  new ObjectError("region", messages.getMessage("message.region.exists", locale));
              result.addError(error);
              break;
  ...
  @PreAuthorize("hasRole('SHIPPING')")
  @RequestMapping(
      value = "/admin/shipping/saveweightBasedShippingMethod.html",
  ...
  public String saveShippingMethod(
    ...
    try {
      ...
      CustomShippingQuotesConfiguration dbConfig =
          (CustomShippingQuotesConfiguration)
              shippingService.getCustomShippingConfiguration(
                  this.WEIGHT_BASED_SHIPPING_METHOD, store);

      shippingService.saveShippingQuoteModuleConfiguration(configuration, store);
      if (dbConfig != null) {
        dbConfig.setActive(configuration.isActive());
        shippingService.saveCustomShippingConfiguration(
            WEIGHT_BASED_SHIPPING_METHOD, dbConfig, store);
      } else {
        shippingService.saveCustomShippingConfiguration(
            WEIGHT_BASED_SHIPPING_METHOD, configuration, store);
      }
  ...
  @PreAuthorize("hasRole('SHIPPING')")
  @RequestMapping(
      value = "/admin/shipping/weightBased/removeCountry.html",
      method = RequestMethod.POST)
  public @ResponseBody ResponseEntity<String> deleteCountry(
  ...
  @PreAuthorize("hasRole('SHIPPING')")
  @RequestMapping(
      value = "/admin/shipping/weightBased/removePrice.html",
      method = RequestMethod.POST)
  public @ResponseBody ResponseEntity<String> deletePrice(
      HttpServletRequest request, HttpServletResponse response, Locale locale) {
      ...
      if (customConfiguration != null) {
        ...
        for (CustomShippingQuotesRegion quote : quotes) {
          ...
          if (quote.getCustomRegionName().equals(region)) {
            ...
            if (quoteItems != null) {
              List<CustomShippingQuoteWeightItem> newQuoteItems =
                  new ArrayList<CustomShippingQuoteWeightItem>();
  ...
  @PreAuthorize("hasRole('SHIPPING')")
  @RequestMapping(
      value = "/admin/shipping/deleteWeightBasedShippingMethod.html",
      method = RequestMethod.POST)
  public String deleteShippingMethod(
    ...
    this.setMenu(model, request);
    MerchantStore store = (MerchantStore) request.getAttribute(Constants.ADMIN_STORE);
    shippingService.removeCustomShippingQuoteModuleConfiguration(
        WEIGHT_BASED_SHIPPING_METHOD, store);
  ...
  public @ResponseBody ResponseEntity<String> checkRegionCode(
      HttpServletRequest request, HttpServletResponse response, Locale locale) {
  ...
  public @ResponseBody ResponseEntity<String> pageCustomShipping(
  ...
  public String editCustomShipping(
  ...
  @PreAuthorize("hasRole('SHIPPING')")
  @RequestMapping(
      value = "/admin/shipping/weightBasedDetails/page.html",
      method = RequestMethod.POST)
  public @ResponseBody ResponseEntity<String> pageCustomShippingDetails(
      HttpServletRequest request, HttpServletResponse response) {
  ...
  public String addPrice(
      @ModelAttribute("region") String customRegion,
    ...
    try {
      ...
      ObjectError error =
          new ObjectError("priceText", messages.getMessage("message.invalid.price", locale));
      result.addError(error);
    }
    ...
    if (weight <= 0) {
      ObjectError error =
          new ObjectError(
              "maximumWeight", messages.getMessage("message.maximumWeight.invalid", locale));
      result.addError(error);
    }
  ...
  @PreAuthorize("hasRole('SHIPPING')")
  @RequestMapping(
      value = "/admin/shipping/weightBased/deleteRegion.html",
      method = RequestMethod.POST)
  ...
  private void populateModel(Model model, HttpServletRequest request, HttpServletResponse response)
    ...
    Map<String, IntegrationConfiguration> configuredModules =
        shippingService.getShippingModulesConfigured(store);
    ...
    environments.add(
        com.salesmanager.core.business.constants.Constants
            .PRODUCTION_ENVIRONMENT); // only production
