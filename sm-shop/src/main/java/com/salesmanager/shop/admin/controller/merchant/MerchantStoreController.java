...
public class MerchantStoreController {
  ...
  private static final String NEW_STORE_TMPL = "email_template_new_store.ftl";
  ...
  public @ResponseBody ResponseEntity<String> pageStores(
  ...
  public String displayMerchantStoreCreate(
      Model model, HttpServletRequest request, HttpServletResponse response, Locale locale)
      throws Exception {
  ...
  public String displayMerchantStore(
      Model model, HttpServletRequest request, HttpServletResponse response, Locale locale)
      throws Exception {
  ...
  private String displayMerchantStore(
      MerchantStore store,
      Model model,
      HttpServletRequest request,
      HttpServletResponse response,
  ...
  public String saveMerchantStore(
      @Valid @ModelAttribute("store") MerchantStore store,
    ...
    if (!StringUtils.isBlank(store.getDateBusinessSince())) {
      try {
        ...
        ObjectError error =
            new ObjectError(
                "dateBusinessSince", messages.getMessage("message.invalid.date", locale));
    ...
    if ((zonesList == null || zonesList.size() == 0)
        && StringUtils.isBlank(store.getStorestateprovince())) {

      ObjectError error =
          new ObjectError("zone.code", messages.getMessage("merchant.zone.invalid", locale));
      result.addError(error);
    }
      ...
      try {
        ...
        Map<String, String> templateTokens =
            emailUtils.createEmailObjectsMap(
                request.getContextPath(), store, messages, storeLocale);
        templateTokens.put(
            EmailConstants.EMAIL_NEW_STORE_TEXT,
            messages.getMessage("email.newstore.text", storeLocale));
        templateTokens.put(
            EmailConstants.EMAIL_STORE_NAME,
            messages.getMessage(
                "email.newstore.name", new String[] {store.getStorename()}, storeLocale));
        templateTokens.put(
            EmailConstants.EMAIL_ADMIN_STORE_INFO_LABEL,
  ...
  public @ResponseBody ResponseEntity<String> checkStoreCode(
  ...
  public @ResponseBody ResponseEntity<String> removeMerchantStore(
