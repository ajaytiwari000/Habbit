...
public class ProductImagesController {
  ...
  public String displayProductImages(
  ...
  public String displayProductImagesUrl(
      @RequestParam("id") long productId,
      Model model,
      HttpServletRequest request,
      HttpServletResponse response)
  ...
  public @ResponseBody ResponseEntity<String> pageProductImages(
    ...
    try {
      ...
      if (images != null) {
        ...
        for (ProductImage image : images) {
          ...
          String imagePath =
              imageUtils.buildProductImageUtils(store, product, image.getProductImage());

          Map entry = new HashMap();
          // entry.put("picture", new
          // StringBuilder().append(request.getContextPath()).append(imagePath).toString());
          entry.put("picture", imagePath);
          entry.put("name", image.getProductImage());
          entry.put("id", image.getId());
          entry.put(
              "defaultImageCheckmark",
              image.isDefaultImage()
                  ? "/resources/img/admin/checkmark_checked.png"
  ...
  public @ResponseBody ResponseEntity<String> pageProductImagesUrl(
      HttpServletRequest request, HttpServletResponse response) {
  ...
  public String saveProductImages(
    ...
    if (product == null) {
      FieldError error =
          new FieldError("productImages", "image", messages.getMessage("message.error", locale));
    ...
    if (CollectionUtils.isNotEmpty(productImages.getFile())) {
      LOGGER.info(
          "Saving {} content images for merchant {}",
          productImages.getFile().size(),
          store.getId());
  ...
  public @ResponseBody ResponseEntity<String> deleteImage(
    ...
    try {
      ...
      if (productImage.getProduct().getMerchantStore().getId().intValue()
  ...
  public @ResponseBody ResponseEntity<String> setDefaultImage(
    ...
    try {
      ...
      final Set<ProductImage> images =
          productService.getById(productImage.getProduct().getId()).getImages();
