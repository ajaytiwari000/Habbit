...
import com.salesmanager.core.business.services.system.EmailsService;
...
public class CustomerFacadeImpl implements CustomerFacade {
   ...
   * (non-Javadoc)
   *
   * @see com.salesmanager.web.shop.controller.customer.language#mergeCart(final Customer
  ...
  public Customer getCustomerByUserName(String userName, MerchantStore store) {
    return customerService.getByUserName(userName, store.getId());
  }
  ...
  private Customer getCustomerByNickAndStoreId(String userName, MerchantStore merchantStore) {
    return Optional.ofNullable(customerService.getByUserName(userName, merchantStore.getId()))
  ...
  public boolean checkIfUserExists(final String userName, final MerchantStore store)
    ...
    if (StringUtils.isNotBlank(userName) && store != null) {
      Customer customer = customerService.getByUserName(userName, store.getId());
      if (customer != null) {
        LOGGER.info(
      ...
      LOGGER.info(
          "No customer found with userName {} for store {} ", userName, store.getStorename());
      return false;
    }
  ...
  public PersistableCustomer registerCustomer(
    ...
    if (customerModel == null) {
      LOGGER.error("Unable to create customer in system");
    ...
    customer.setFirstOrderPlaced(customerModel.isFirstOrderPlaced());
    customer.setReferralCodeApplied(customerModel.isReferralCodeApplied());
    return customer;
  }
  ...
  public Customer getCustomerModel(
    ...
    // set groups
    if (!StringUtils.isBlank(customerModel.getPassword())
        && !StringUtils.isBlank(customerModel.getUserName())) {
  ...
  @Override
  public PersistableCustomer getCustomerById(final Long id, final MerchantStore store) {
    Customer customerModel = null;
    try {
      customerModel = customerService.getById(id);
    } catch (Exception e) {
      throwServiceRuntImeException(
          e,
          CustomerErrorCodes.CUSTOMER_GET_BY_ID_FAILURE.getErrorCode(),
          CustomerErrorCodes.CUSTOMER_GET_BY_ID_FAILURE.getErrorMessage());
    }
    PersistableCustomer persistableCustomer = new PersistableCustomer();
    try {
      customerPopulator.populateToPersistable(customerModel, persistableCustomer);
    } catch (ConversionException e) {
      throwServiceRuntImeException(
          e,
          CustomerErrorCodes.CUSTOMER_POPULATOR_FAILURE.getErrorCode(),
          CustomerErrorCodes.CUSTOMER_POPULATOR_FAILURE.getErrorMessage());
    }
    return persistableCustomer;
  }

  @Override
  public PersistableCustomer getCustomerByUserName(String userName) {
    Customer customerModel = null;
    try {
      customerModel = customerService.getByUserName(userName);
  ...
  public PersistableCustomer create(PersistableCustomer customer, MerchantStore store) {
    ...
    Customer cusExist = null;
    try {
      cusExist = customerService.getByUserName(customer.getUserName());
    } catch (ServiceException e) {
      throwServiceRuntImeException(
          e,
          CustomerErrorCodes.CUSTOMER_GET_BY_USERNAME_FAILURE.getErrorCode(),
    ...
    if (Objects.nonNull(cusExist)) {
      customer.setId(cusExist.getId());
      createMembership(customer);
      return update(customer, store);
    }
  ...
  // TODO : Need to move it to separate class
  private void createMembership(PersistableCustomer customer) {
    PersistableMembership persistableMembership = membershipPopulator(customer);
    persistableMembership = membershipFacade.create(persistableMembership, null);
    LOGGER.info("Membership created for user : {}", persistableMembership.getPhoneNumber());
    if (StringUtils.isNotEmpty(customer.getEmailAddress())) {
      sendSuccessfulRegistrationEmail(customer);
    }
  }

  private PersistableMembership membershipPopulator(PersistableCustomer customer) {
    PersistableMembership persistableMembership = new PersistableMembership();
    persistableMembership.setRewardPoint(ProductRewardPoint.PROFILE_CREATION_POINT.getValue());
    persistableMembership.setSuccessfulReferrals(0);
    persistableMembership.setEarnedThroughReferrals(0);
    PersistableReferralCode persistableReferralCode = null;
    if (StringUtils.isNotEmpty(customer.getReferralCode())) {
      persistableReferralCode =
          referralCodeFacade.getReferralCodeByCode(customer.getReferralCode());
      // no point will be rewarded in case of ADMIN
      if (Objects.nonNull(persistableReferralCode)
          && !OwnerType.ADMIN.name().equals(persistableReferralCode.getOwnerType().name())) {
        customer.setFriendPhone(persistableReferralCode.getPhoneNumber()); // friend phone number
        updateCartWithCustomer(
            customer, persistableReferralCode.getPhoneNumber()); // friend phone, name
        addPointToReferrer(persistableReferralCode);
      }
      customer.setReferralCodeApplied(true);
      persistableMembership.setRewardPoint(
          persistableReferralCode.getRefereePoints() + persistableMembership.getRewardPoint());
    }
    persistableMembership.setPhoneNumber(customer.getPhoneNumber());
    persistableMembership.setTierType(TierType.HABBIT_BLUE);
    persistableMembership.setStartDateOfCurTier(setStartDate());
    persistableMembership.setEndDateOfCurTier(setEndDate());
    persistableMembership.setEarnedPointCurTier(persistableMembership.getRewardPoint());
    persistableMembership.setLiftTimePoint(persistableMembership.getRewardPoint());
    persistableMembership.setMinDowngradePoint(TierType.HABBIT_BLUE.getLowerLimit());
    persistableMembership.setMinUpgradePoint(TierType.HABBIT_GOLD.getLowerLimit());

    return persistableMembership;
  }

  private void updateCartWithCustomer(PersistableCustomer customer, String friendPhoneNumber) {
    Cart cart = cartFacade.getCartByPhone(customer.getPhoneNumber());
    cart.setCustomerName(customer.getFirstName() + " " + customer.getLastName());
    cart.setFriendPhone(friendPhoneNumber);
  ...
  private void addPointToReferrer(PersistableReferralCode persistableReferralCode) {
    Membership membership = null;
    try {
      membership = membershipService.getMembershipByPhone(persistableReferralCode.getPhoneNumber());
    } catch (Exception e) {
      throwServiceRuntImeException(
          e,
          ...
          AttributeErrorCodes.MEMBERSHIP_GET_BY_PHONE_FAILURE.getErrorMessage()
              + persistableReferralCode.getPhoneNumber());
    }
    if (Objects.isNull(membership)) {
      LOGGER.error(
          "Membership with user - {} not found in DB.", persistableReferralCode.getPhoneNumber());
      throw new ResourceNotFoundException(
          "Membership with user  [" + persistableReferralCode.getPhoneNumber() + " ] not found");
    }

    membership.setLiftTimePoint(
        membership.getLiftTimePoint() + persistableReferralCode.getReferrerPoints());
    membership.setEarnedPointCurTier(
        membership.getEarnedPointCurTier() + persistableReferralCode.getReferrerPoints());
    membership.setEarnedThroughReferrals(
        membership.getEarnedThroughReferrals() + persistableReferralCode.getReferrerPoints());
    membership.setRewardPoint(
        persistableReferralCode.getReferrerPoints() + membership.getRewardPoint());
    membership.setSuccessfulReferrals(membership.getSuccessfulReferrals() + 1);
    membershipFacade.updateTier(membership);
    try {
  ...
  private void saveCustomer(Customer customerToPopulate) {
    try {
      ...
      throwServiceRuntImeException(
          e,
          CustomerErrorCodes.CUSTOMER_UPDATE_FAILURE.getErrorCode(),
          CustomerErrorCodes.CUSTOMER_UPDATE_FAILURE.getErrorMessage());
    }
  }

  private boolean userExist(String userName) {
    try {
      return Optional.ofNullable(customerService.getByUserName(userName)).isPresent();
    } catch (ServiceException e) {
      throwServiceRuntImeException(
          e,
          CustomerErrorCodes.CUSTOMER_GET_BY_USERNAME_FAILURE.getErrorCode(),
  ...
  public PersistableCustomer update(PersistableCustomer customer, MerchantStore store) {
    ...
    Customer cust = null;
    try {
      cust = customerService.getById(customer.getId());
    } catch (Exception e) {
      throwServiceRuntImeException(
          e,
          CustomerErrorCodes.CUSTOMER_GET_BY_ID_FAILURE.getErrorCode(),
    ...
    try {
      if (!StringUtils.isEmpty(customer.getReferralCode()) && !customer.isReferralCodeApplied()) {
        updateMembership(customer);
      }
      customerPopulator.populate(customer, cust, store, store.getDefaultLanguage());
      addCustomerNotification(cust);
    ...
    customer.setEmailVerified(cust.isEmailVerified());
    customer.setProfileCompleted(cust.isProfileCompleted());
    customer.setFirstOrderPlaced(cust.isFirstOrderPlaced());
    customer.setReferralCodeApplied(cust.isReferralCodeApplied());
    return customer;
  }

  private void addCustomerNotification(Customer cust) {
    PersistableCustomerNotification customerNotification = new PersistableCustomerNotification();
    customerNotification.setWhatsAppEnable(true);
    customerNotification.setSmsEnable(true);
    customerNotification.setEmailEnable(true);
    customerNotification.setPhone(cust.getPhoneNumber());
    cust.setCustomerNotification(
        customerNotificationFacade.createCustomerNotification(customerNotification));
  }

  private void sendSuccessfulRegistrationEmail(PersistableCustomer customer) {
    Email email = emailService.createMail(EmailType.SUCCESSFUL_REGISTRATION);
    email.setTo(customer.getEmailAddress());
    email
        .getTemplateTokens()
        .put("ConsumerName", customer.getFirstName() + " " + customer.getLastName());
    try {
      emailService.sendEmail(email);
    } catch (Exception e) {
      throwServiceRuntImeException(
          e,
          CustomerErrorCodes.CUSTOMER_SUCCESSFUL_REGISTRATION_EMAIL_FAILURE.getErrorCode(),
          CustomerErrorCodes.CUSTOMER_SUCCESSFUL_REGISTRATION_EMAIL_FAILURE.getErrorMessage());
    }
  }

  private void updateMembership(PersistableCustomer customer) {
    // TODO : Need to move it to separate class
    PersistableMembership persistableMembership =
        membershipFacade.getMembershipByPhone(customer.getPhoneNumber());
    PersistableReferralCode customerReferralCode =
        referralCodeFacade.getReferralCodeByPhone(customer.getPhoneNumber());
    if (customerReferralCode.getCodeName().equals(customer.getReferralCode())) {
      throwServiceRuntImeException(
          new Exception(),
          AttributeErrorCodes.REFERRAL_CODE_APPLY_OWN.getErrorCode(),
          AttributeErrorCodes.REFERRAL_CODE_APPLY_OWN.getErrorMessage());
    }
    PersistableReferralCode persistableReferralCode =
        referralCodeFacade.getReferralCodeByCode(customer.getReferralCode());
    customer.setFriendPhone(persistableReferralCode.getPhoneNumber()); // friend phone number
    updateCartWithCustomer(
        customer, persistableReferralCode.getPhoneNumber()); // update cart friend phone number
    customer.setReferralCodeApplied(true);
    // no point will be rewarded in case of ADMIN
    if (Objects.nonNull(persistableReferralCode)
        && !OwnerType.ADMIN.name().equals(persistableReferralCode.getOwnerType().name())) {
    ...
    persistableMembership.setRewardPoint(
        persistableMembership.getRewardPoint() + persistableReferralCode.getRefereePoints());
    persistableMembership.setEarnedPointCurTier(
        persistableMembership.getEarnedPointCurTier() + persistableReferralCode.getRefereePoints());
    persistableMembership.setLiftTimePoint(
        persistableMembership.getLiftTimePoint() + persistableReferralCode.getRefereePoints());
    Membership membership = membershipMapper.toMembership(persistableMembership);
    membershipFacade.updateTier(membership);
    membershipFacade.update(membershipMapper.toPersistableMembership(membership), null);

    LOGGER.info("Membership updated for user : {}", persistableMembership.getPhoneNumber());
  }
  ...
  private Customer getCustomerById(Long customerId) {
    return Optional.ofNullable(customerService.getById(customerId))
        .orElseThrow(
            () -> new ResourceNotFoundException("Customer id " + customerId + " does not exists"));
  }
  ...
  public void resetPassword(Customer customer, MerchantStore store, Language language)
    ...
    try {
      ...
      emailsService.sendHtmlEmail(store, email);
  ...
  @Override
  public PersistableCustomer uploadImage(
      MultipartFile[] files, MerchantStore merchantStore, Long id) {
    Customer customer = null;
    try {
      customer = customerService.addCustomerImage(id, files);
    } catch (ServiceException e) {
      throwServiceRuntImeException(
          e,
          CustomerErrorCodes.CUSTOMER_IMAGE_CREATE_FAILURE.getErrorCode(),
          CustomerErrorCodes.CUSTOMER_IMAGE_CREATE_FAILURE.getErrorMessage());
    }
    PersistableCustomer persistableCustomer = new PersistableCustomer();
    try {
      customerPopulator.populateToPersistable(customer, persistableCustomer);
    } catch (ConversionException e) {
      throwServiceRuntImeException(
          e,
          CustomerErrorCodes.CUSTOMER_POPULATOR_FAILURE.getErrorCode(),
  ...
  public void removeCustomerImage(Long id) {
    ...
    try {
      ...
      throwServiceRuntImeException(
          e,
          CustomerErrorCodes.CUSTOMER_GET_BY_ID_FAILURE.getErrorCode(),
          CustomerErrorCodes.CUSTOMER_GET_BY_ID_FAILURE.getErrorMessage() + id);
    }
    try {
      customerService.removeCustomerImage(customer);
    } catch (Exception e) {
      throwServiceRuntImeException(
          e,
          CustomerErrorCodes.CUSTOMER_IMAGE_DELETE_FAILURE.getErrorCode(),
          CustomerErrorCodes.CUSTOMER_IMAGE_DELETE_FAILURE.getErrorMessage() + id);
    }
  }

  @Override
  public boolean getValidateReferralCode(String referralCode, HttpServletRequest request) {
    PersistableReferralCode persistableReferralCode =
        referralCodeFacade.getReferralCodeByCode(referralCode);
    if (Objects.nonNull(persistableReferralCode)) {
      return true;
    }
    return false;
  ...
  @Override
  public String updateEmail(Long id, String customerEmail, MerchantStore merchantStore) {
    Customer customer = null;
    try {
      customer = customerService.getById(id);
    } catch (Exception e) {
  ...
  @Override
  public boolean newUser(String phone) {
    Customer customerModel = null;
    try {
    ...
    return Objects.isNull(customerModel) ? true : false;
  }

  @Override
  public String emailVerification(String phone, String secret) {
    BCryptPasswordEncoder secretCode = (BCryptPasswordEncoder) passwordEncoder;
    if (!secretCode.matches(phone + HabbitCoreConstant.SECRET, secret)) {
      return EmailConstant.NOT_VERIFIED;
    }
    Customer customer = getByNick(phone);
    if (customer.isEmailVerified()) {
      return EmailConstant.ALREADY_VERIFIED;
    }
