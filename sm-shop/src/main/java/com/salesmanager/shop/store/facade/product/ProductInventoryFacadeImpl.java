...
public class ProductInventoryFacadeImpl implements ProductInventoryFacade {
  ...
  @Autowired private ProductAvailabilityService productAvailabilityService;

  @Autowired private ProductService productService;

  @Autowired private MerchantStoreService merchantStoreService;

  @Autowired private ReadableInventoryMapper readableInventoryMapper;

  @Autowired private PersistableInventoryMapper productInventoryMapper;

  @Override
  public ReadableInventoryList getInventory(
      Long productId, MerchantStore store, String child, Language language, int page, int count) {
  ...
  public ReadableInventory get(Long productId, String child, Language language) {
    ...
    try {
      ...
      if (store.getParent() == null
          || store.getParent().getId().intValue()
              != product.getMerchantStore().getId().intValue()) {
        throw new ResourceNotFoundException(
  ...
  public ReadableInventory add(
      Long productId, PersistableInventory inventory, MerchantStore store, Language language) {
  ...
  public ReadableInventory get(
      Long productId, Long inventoryId, MerchantStore store, Language language) {
    try {
      Product product = productService.getById(productId);
