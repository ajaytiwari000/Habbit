...
public class JWTTokenUtil implements Serializable {
  ...
  private Boolean isCreatedBeforeLastPasswordResetWithGrace(Date created, Date lastPasswordReset) {
    return (lastPasswordReset != null
        && created.before(addSeconds(lastPasswordReset, GRACE_PERIOD)));
  }
  ...
  public String generateToken(UserDetails userDetails, String requestSource) {
    Map<String, Object> claims = new HashMap<>();
    return doGenerateToken(claims, userDetails.getUsername(), generateAudience(), requestSource);
  }

  private String doGenerateToken(
      Map<String, Object> claims, String subject, String audience, String requestSource) {
    final Date createdDate = DateUtil.getDate();
    final Date expirationDate =
        calculateExpirationDate(createdDate, requestSource); // valid for next 2 month
  ...
  public String refreshToken(String token) {
    final Date createdDate = DateUtil.getDate();
    final Date expirationDate = calculateExpirationDate(createdDate, null);
  ...
  private Date calculateExpirationDate(Date createdDate, String requestSource) {
    Date expiryTime = new Date(createdDate.getTime() + expiration * 1000);
    if ("IOS".equals(requestSource) || "ANDROID".equals(requestSource)) {
      expiryTime = new Date(createdDate.getTime() + mobileExpiration * 1000);
    }
    return expiryTime;
  }
}
