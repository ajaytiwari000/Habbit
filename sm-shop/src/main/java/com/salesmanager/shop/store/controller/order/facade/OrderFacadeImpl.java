...
import com.salesmanager.core.model.order.payment.CreditsCard;
...
public class OrderFacadeImpl implements OrderFacade {
  ...
  @Override
  public ShopOrder initializeOrder(
      MerchantStore store, Customer customer, ShoppingCart shoppingCart, Language language)
      throws Exception {
  ...
  @Override
  public OrderTotalSummary calculateOrderTotal(
      MerchantStore store, ShopOrder order, Language language) throws Exception {
  ...
  @Override
  public OrderTotalSummary calculateOrderTotal(
      MerchantStore store, PersistableOrder order, Language language) throws Exception {
  ...
  private OrderTotalSummary calculateOrderTotal(
      MerchantStore store, Customer customer, PersistableOrder order, Language language)
      throws Exception {
  ...
  private PersistableCustomer persistableCustomer(
    ...
    PersistableCustomer persistableCustomer =
        customerPopulator.populate(customer, new PersistableCustomer(), store, language);
  ...
  public Order processOrder(
      ShopOrder order, Customer customer, MerchantStore store, Language language)
  ...
  private Order processOrderModel(
      ShopOrder order,
      Customer customer,
      Transaction transaction,
      MerchantStore store,
    ...
    try {
      ...
      modelOrder.setLocale(
          LocaleUtils.getLocale(
              store)); // set the store locale based on the country for order $ formatting
      ...
      Collections.sort(
          ...
          new Comparator<com.salesmanager.core.model.order.OrderTotal>() {
            public int compare(
                com.salesmanager.core.model.order.OrderTotal x,
                com.salesmanager.core.model.order.OrderTotal y) {
              if (x.getSortOrder() == y.getSortOrder()) return 0;
              return x.getSortOrder() < y.getSortOrder() ? -1 : 1;
            }
          });

      Set<com.salesmanager.core.model.order.OrderTotal> modelTotals =
          new LinkedHashSet<com.salesmanager.core.model.order.OrderTotal>();
      ...
      if (PaymentType.CREDITCARD.name().equals(paymentType)) {
        ...
        if (creditCardType != null) {
          ...
          CreditsCard cc = new CreditsCard();
          ...
          cc.setCcExpires(
              ((CreditCardPayment) payment).getExpirationMonth()
                  + "-"
          ...
          if (!StringUtils.isBlank(cc.getCcNumber())) {
            String maskedNumber =
                CreditCardUtils.maskCardNumber(order.getPayment().get("creditcard_card_number"));
      ...
      if (transaction != null) {
        orderService.processOrder(
            modelOrder, customer, order.getShoppingCartItems(), summary, payment, store);
      } else {
        orderService.processOrder(
            modelOrder,
            customer,
  ...
  public ShippingQuote getShippingQuote(
    ...
    ShippingQuote quote =
        shippingService.getShippingQuote(cart.getId(), store, delivery, shippingProducts, language);
  ...
  public ShippingSummary getShippingSummary(
      ShippingQuote quote, MerchantStore store, Language language) {
  ...
  public void validateOrder(
      ...
      Map<String, String> messagesResult,
      MerchantStore store,
      Locale locale)
      throws ServiceException {
    ...
    try {
      ...
      //        bindingResult.addError(error);
      //        messagesResult.put(
      //            "customer.billing.lastName", messages.getMessage("NotEmpty.customer.lastName",
      // locale));
      //      }
      ...
      if (StringUtils.isBlank(order.getCustomer().getEmailAddress())) {
        ...
        bindingResult.addError(error);
        messagesResult.put(
            "customer.emailAddress", messages.getMessage("NotEmpty.customer.emailAddress", locale));
      }
      ...
      //        bindingResult.addError(error);
      //        messagesResult.put(
      //            "customer.billing.city", messages.getMessage("NotEmpty.customer.billing.city",
      // locale));
      //      }
      //
      ...
      if (paymentType == null) {
        ServiceException serviceException =
            new ServiceException(ServiceException.EXCEPTION_VALIDATION, "payment.required");
      ...
      if (shippingService.requiresShipping(order.getShoppingCartItems(), store)
        ...
        ServiceException serviceException =
            new ServiceException(ServiceException.EXCEPTION_VALIDATION, "shipping.required");
        throw serviceException;
      }
      ...
      if (PaymentType.CREDITCARD.name().equals(paymentType)
        ...
        if (StringUtils.isBlank(cco)
            || StringUtils.isBlank(cvv)
            || StringUtils.isBlank(ccn)
            || StringUtils.isBlank(ccm)
            || StringUtils.isBlank(ccd)) {
          ObjectError error =
              new ObjectError(
                  "creditcard", messages.getMessage("messages.error.creditcard", locale));
          bindingResult.addError(error);
          messagesResult.put(
              "creditcard", messages.getMessage("messages.error.creditcard", locale));
          return;
        ...
        if (creditCardType == null) {
          ServiceException serviceException =
              new ServiceException(ServiceException.EXCEPTION_VALIDATION, "cc.type");
          throw serviceException;
        }
      }
  ...
  @Override
  public ReadableOrderList getReadableOrderList(
  ...
  private ReadableOrderList populateOrderList(
  ...
  private void setOrderProductList(
      ...
      final ReadableOrder readableOrder)
  ...
  private ReadableOrderList getReadableOrderList(
  ...
  @Override
  public ShippingQuote getShippingQuote(
      Customer customer, ShoppingCart cart, MerchantStore store, Language language)
      throws Exception {
    ...
    //        || StringUtils.isBlank(customer.getDelivery().getPostalCode())) {
    //      Billing billing = customer.getBilling();
    //      delivery.setAddress(billing.getAddress());
    //      delivery.setCity(billing.getCity());
    //      delivery.setCompany(billing.getCompany());
  ...
  public Order processOrder(
    ...
    try {
      ...
      // requires Order Totals, this needs recalculation and then compare total with the amount sent
      // as part
      ...
      Set<com.salesmanager.core.model.order.OrderTotal> set =
          new HashSet<com.salesmanager.core.model.order.OrderTotal>();
        ...
        try {
          ...
          if (orderService.hasDownloadFiles(modelOrder)) {
            emailTemplatesUtils.sendOrderDownloadEmail(
                customer, modelOrder, store, locale, coreConfiguration.getProperty("CONTEXT_PATH"));
          }
  ...
  @Override
  public ReadableOrderList getCapturableOrderList(
      MerchantStore store, Date startDate, Date endDate, Language language) throws Exception {
  ...
  @Override
  public ReadableTransaction captureOrder(
      MerchantStore store, Order order, Customer customer, Language language) throws Exception {
