...
public class ShoppingOrderController extends AbstractController {
  ...
  public String displayCheckout(
      @CookieValue("cart") String cookie,
      Model model,
     ...
     * Shopping cart
     *
     * <p>ShoppingCart should be in the HttpSession Otherwise the cart id is in the cookie Otherwise
     * the customer is in the session and a cart exist in the DB Else -> Nothing to display
     */
    ...
    Set<com.salesmanager.core.model.shoppingcart.ShoppingCartItem> availables =
    ...
    if (customer != null) {
      ...
      // countriesMap.get(anonymousCustomer.getBilling().getCountry());
    ...
    if (quote != null) {
      ...
      if (StringUtils.isBlank(shippingReturnCode)
          || shippingReturnCode.equals(ShippingQuote.NO_POSTAL_CODE)) {
          ...
          for (ShippingOption shipOption : options) {
            ...
            if (!StringUtils.isBlank(shipOption.getOptionCode())) {
              ...
              try {
                ...
                LOGGER.warn(
                    "displayCheckout No shipping code found for " + optionCodeBuilder.toString());
              }
            }
          }
        }
      ...
      if (quote.getDeliveryAddress() != null) {
        ReadableCustomerDeliveryAddressPopulator addressPopulator =
            new ReadableCustomerDeliveryAddressPopulator();
    ...
    if (quote != null
        && quote.getShippingReturnCode() != null
      ...
      model.addAttribute(
          "errorMessages",
          messages.getMessage(
              quote.getShippingReturnCode(), locale, quote.getShippingReturnCode()));
    }
    ...
    if (CollectionUtils.isEmpty(paymentMethods) && !freeShoppingCart) {
      ...
      model.addAttribute(
          "errorMessages",
          messages.getMessage("payment.not.configured", locale, "No payments configured"));
    }
    ...
    StringBuilder template =
        new StringBuilder()
            .append(ControllerConstants.Tiles.Checkout.checkout)
            .append(".")
  ...
  public String commitPreAuthorizedOrder(
    ...
    if (order == null) {
      StringBuilder template =
          new StringBuilder()
              .append(ControllerConstants.Tiles.Pages.timeout)
              .append(".")
              .append(store.getStoreTemplate());
      return template.toString();
    }
    ...
    Map<String, Object> configs =
        (Map<String, Object>) request.getAttribute(Constants.REQUEST_CONFIGS);
  ...
  private Order commitOrder(ShopOrder order, HttpServletRequest request, Locale locale)
    ...
    if (auth != null && request.isUserInRole("AUTH_CUSTOMER")) {
      ...
      customer.setUserName(authCustomer.getUserName());
    ...
    try { // set groups
      if (authCustomer == null) { // not authenticated, create a new volatile user
        ...
        userName = modelCustomer.getUserName();
        ...
        modelCustomer =
            customerFacade.populateCustomerModel(authCustomer, customer, store, language);
      }
    } catch (Exception e) {
    ...
    Transaction initialTransaction =
        (Transaction) super.getSessionAttribute(Constants.INIT_TRANSACTION_KEY, request);
    if (initialTransaction != null) {
      modelOrder =
          orderFacade.processOrder(order, modelCustomer, initialTransaction, store, language);
    } else {
    ...
    try {
      // refresh customer --
      modelCustomer = customerFacade.getCustomerByUserName(modelCustomer.getUserName(), store);

      // if has downloads, authenticate

      // check if any downloads exist for this order6
      List<OrderProductDownload> orderProductDownloads =
          orderProdctDownloadService.getByOrderId(modelOrder.getId());
      if (CollectionUtils.isNotEmpty(orderProductDownloads)) {
        ...
        if (order.getCustomer().getId() == null || order.getCustomer().getId().longValue() == 0) {
          ...
          emailTemplatesUtils.sendRegistrationEmail(
              customer, store, locale, request.getContextPath());
        }
      }

      // send order confirmation email to customer
      emailTemplatesUtils.sendOrderEmail(
          modelCustomer.getEmailAddress(),
          modelCustomer,
          modelOrder,
          locale,
          language,
      ...
      if (orderService.hasDownloadFiles(modelOrder)) {
        emailTemplatesUtils.sendOrderDownloadEmail(
            modelCustomer, modelOrder, store, locale, request.getContextPath());
      }
  ...
  public String commitOrder(
      @CookieValue("cart") String cookie,
      @Valid @ModelAttribute(value = "order") ShopOrder order,
    ...
    try {
      ...
      /**
       * Retrieve shopping cart and metadata (information required to process order)
       *
       * <p>- Cart rerieved from cookie or from user session - Retrieves payment metadata
       */
      ShippingMetaData shippingMetaData = shippingService.getShippingMetaData(store);
      model.addAttribute("shippingMetaData", shippingMetaData);
      // basic stuff
      ...
      /** Prepare failure data - Get another shipping quote */
      ShippingQuote quote =
          orderFacade.getShippingQuote(order.getCustomer(), cart, order, store, language);
      ...
      if (quote != null) {
        ...
        if (!CollectionUtils.isEmpty(options)) {
          ...
          for (ShippingOption shipOption : options) {
            ...
            String carrier =
                messages.getMessage(
                    moduleName.toString(), new String[] {store.getStorename()}, locale);
            ...
            if (!StringUtils.isBlank(shipOption.getOptionCode())) {
              ...
              try {
                ...
                LOGGER.warn(
                    "commitOrder No shipping code found for " + optionCodeBuilder.toString());
              }
            }
          }
        }
      ...
      if (order.getSelectedShippingOption() != null) {
        ShippingSummary summary =
            (ShippingSummary) request.getSession().getAttribute(Constants.SHIPPING_SUMMARY);
        List<ShippingOption> options =
            (List<ShippingOption>) request.getSession().getAttribute(Constants.SHIPPING_OPTIONS);
        ...
        ReadableShippingSummaryPopulator readableSummaryPopulator =
            new ReadableShippingSummaryPopulator();
      ...
      if (se.getExceptionType() == ServiceException.EXCEPTION_VALIDATION) {
        ...
        if (!StringUtils.isBlank(se.getMessageCode())) {
          String messageLabel =
              messages.getMessage(se.getMessageCode(), locale, paymentDeclinedMessage);
          model.addAttribute("errorMessages", messageLabel);
        } else {
  ...
  @SuppressWarnings("unchecked")
  @RequestMapping(
      value = {"/shippingQuotes.json"},
      method = RequestMethod.POST)
  public @ResponseBody ReadableShopOrder calculateShipping(
      @ModelAttribute(value = "order") ShopOrder order,
      HttpServletRequest request,
      HttpServletResponse response,
      Locale locale)
      throws Exception {
    ...
    try {
      ...
      com.salesmanager.core.model.shoppingcart.ShoppingCart cart =
          shoppingCartFacade.getShoppingCartModel(shoppingCartCode, store);
      ...
      if (quote != null) {
            ...
            for (ShippingOption shipOption : options) {
              ...
              if (!StringUtils.isBlank(shipOption.getOptionCode())) {
                ...
                try {
                  ...
                  LOGGER.warn(
                      "calculateShipping No shipping code found for "
        ...
        if (quote.getShippingReturnCode() != null
            && quote
                .getShippingReturnCode()
                .equals(ShippingQuote.NO_SHIPPING_TO_SELECTED_COUNTRY)) {
          if (CollectionUtils.isEmpty(
              quote.getShippingOptions())) { // only if there are no other options
            LOGGER.error("Shipping quote error " + quote.getShippingReturnCode());
            readableOrder.setErrorMessage(messages.getMessage("message.noshipping", locale));
          }
        }
   ...
   */
  @RequestMapping(
      value = {"/calculateOrderTotal.json"},
      method = RequestMethod.POST)
  public @ResponseBody ReadableShopOrder calculateOrderTotal(
      @ModelAttribute(value = "order") ShopOrder order,
      HttpServletRequest request,
      HttpServletResponse response,
      Locale locale)
    ...
    try {
      ...
      ReadableDelivery readableDelivery =
          super.getSessionAttribute(Constants.KEY_SESSION_ADDRESS, request);
      ...
      if (order.getSelectedShippingOption() != null) {
        ...
        if (!CollectionUtils.isEmpty(options)) {
          ...
          for (ShippingOption shipOption : options) {
            ...
            if (!StringUtils.isBlank(shipOption.getOptionCode())) {
              ...
              try {
                ...
                LOGGER.warn(
                    "calculateOrderTotal No shipping code found for "
                        + optionCodeBuilder.toString());
              }
            }
          }
          ...
          Map<String, String> informations =
              (Map<String, String>) request.getSession().getAttribute("SHIPPING_INFORMATIONS");
      ...
      List<ReadableOrderTotal> subtotals = new ArrayList<ReadableOrderTotal>();
      for (OrderTotal total : orderTotalSummary.getTotals()) {
        if (total.getOrderTotalCode() == null
            || !total.getOrderTotalCode().equals("order.total.total")) {
          ReadableOrderTotal t = new ReadableOrderTotal();
          totalPopulator.populate(total, t, store, language);
          subtotals.add(t);
        } else { // grand total
