...
public class ShoppingCategoryController {
  ...
  @Inject private BreadcrumbsUtils breadcrumbsUtils;
  ...
  public String displayCategoryWithReference(
  ...
  public String displayCategoryNoReference(
      @PathVariable final String friendlyUrl,
      Model model,
      HttpServletRequest request,
      HttpServletResponse response,
  ...
  private String displayCategory(
    ...
    ReadableCategorysPopulator populator = new ReadableCategorysPopulator();
    ReadableCategorys categoryProxy =
        populator.populate(category, new ReadableCategorys(), store, language);

    Breadcrumb breadCrumb =
        breadcrumbsUtils.buildCategoryBreadcrumb(
            categoryProxy, store, language, request.getContextPath());
    ...
    String lineage =
        new StringBuilder()
            ...
            .append(Constants.CATEGORY_LINEAGE_DELIMITER)
            .toString();
    ...
    List<ReadableCategorys> subCategories = null;
    ...
    if (store.isUseCache()) {
      ...
      // get from the cache
      subCategories =
          (List<ReadableCategorys>) cache.getFromCache(subCategoriesCacheKey.toString());
      if (subCategories == null) {
        ...
        subCategories =
            getSubCategories(store, category, countProductsByCategories, language, locale);
    ...
    // Parent category
    ReadableCategorys parentProxy = null;

    if (category.getParent() != null) {
      Category parent = categoryService.getById(category.getParent().getId(), store.getId());
      parentProxy = populator.populate(parent, new ReadableCategorys(), store, language);
    }

    // ** List of manufacturers **//
    List<ReadableManufacturer> manufacturerList =
        getManufacturersByProductAndCategory(store, category, subIds, language);
    ...
    StringBuilder template =
        new StringBuilder()
            .append(ControllerConstants.Tiles.Category.category)
            .append(".")
  ...
  @SuppressWarnings("unchecked")
  private List<ReadableManufacturer> getManufacturersByProductAndCategory(
      MerchantStore store, Category category, List<Long> subCategoryIds, Language language)
      throws Exception {
    ...
    if (subCategoryIds != null && subCategoryIds.size() > 0) {
      ...
      if (store.isUseCache()) {
        ...
        manufacturerList =
            (List<ReadableManufacturer>) cache.getFromCache(manufacturersKey.toString());
  ...
  private List<ReadableManufacturer> getManufacturers(
      MerchantStore store, List<Long> ids, Language language) throws Exception {
    List<ReadableManufacturer> manufacturerList = new ArrayList<ReadableManufacturer>();
    List<com.salesmanager.core.model.catalog.product.manufacturer.Manufacturer> manufacturers =
        manufacturerService.listByProductsByCategoriesId(store, ids, language);
    if (!manufacturers.isEmpty()) {

      for (com.salesmanager.core.model.catalog.product.manufacturer.Manufacturer manufacturer :
          manufacturers) {
        ReadableManufacturer manuf =
            new ReadableManufacturerPopulator()
  ...
  private Map<Long, Long> getProductsByCategory(
      MerchantStore store, Category category, String lineage, List<Long> subIds) throws Exception {
    ...
    List<Object[]> countProductsByCategories =
        categoryService.countProductsByCategories(store, subIds);
  ...
  private List<ReadableCategorys> getSubCategories(
      MerchantStore store,
      Category category,
    ...
    ReadableCategorysPopulator populator = new ReadableCategorysPopulator();
    List<ReadableCategorys> subCategoryProxies = new ArrayList<ReadableCategorys>();

    for (Category sub : subCategories) {
      ReadableCategorys cProxy = populator.populate(sub, new ReadableCategorys(), store, language);
      // com.salesmanager.web.entity.catalog.Category cProxy =  catalogUtils.buildProxyCategory(sub,
      // store, locale);
  ...
  public List<ReadableCategorys> getCategories(
    ...
    List<ReadableCategorys> returnCategories = new ArrayList<ReadableCategorys>();
    for (Category category : categories) {
      ReadableCategorys categoryProxy =
          populator.populate(category, new ReadableCategorys(), merchantStore, l);
  ...
  public ProductList getProducts(
    ...
    try {
      ...
      if (merchantStore == null) {
        LOGGER.error("Merchant store is null for code " + store);
        response.sendError(
            503, "Merchant store is null for code " + store); // TODO localized message
        return null;
      }
      ...
      String lineage =
          new StringBuilder().append(cat.getLineage()).append(cat.getId()).append("/").toString();
      ...
      List<com.salesmanager.core.model.catalog.product.Product> products =
          productService.getProducts(ids, lang);
      ...
      for (Product product : products) {
        ...
        ReadableProducts p =
            populator.populate(product, new ReadableProducts(), merchantStore, lang);
   ...
   */
  @RequestMapping(
      "/services/public/products/page/{start}/{max}/{store}/{language}/{category}/filter={filterType}/filter-value={filterValue}")
  @ResponseBody
  public ProductList getProductsFilteredByType(
      @PathVariable int start,
      @PathVariable int max,
      @PathVariable String store,
      @PathVariable final String language,
      @PathVariable final String category,
      @PathVariable final String filterType,
      @PathVariable final String filterValue,
      Model model,
      HttpServletRequest request,
      HttpServletResponse response)
      throws Exception {
    ...
    return this.getProducts(
        start, max, store, language, category, queryFilters, model, request, response);
  }

  private ProductList getProducts(
      final int start,
      final int max,
      final String store,
    ...
    try {
      ...
      com.salesmanager.core.model.catalog.product.ProductList products =
          productService.listByStore(merchantStore, lang, productCriteria);
      ...
      Collections.sort(
          ...
          new Comparator<ReadableProducts>() {

            @Override
            public int compare(ReadableProducts o1, ReadableProducts o2) {
