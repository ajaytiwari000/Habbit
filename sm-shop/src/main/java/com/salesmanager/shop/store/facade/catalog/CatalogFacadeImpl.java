...
public class CatalogFacadeImpl implements CatalogFacade {
  ...
  @Autowired private CatalogEntryService catalogEntryService;

  @Autowired private PersistableCatalogMapper persistableCatalogMapper;

  @Autowired private ReadableCatalogMapper readableCatalogMapper;

  @Autowired private PersistableCatalogEntryMapper persistableCatalogEntryMapper;

  @Autowired private ReadableCatalogEntryMapper readableCatalogEntryMapper;
  ...
  public void deleteCatalog(Long catalogId, MerchantStore store, Language language) {
    ...
    if (c.getMerchantStore() != null && !c.getMerchantStore().getCode().equals(store.getCode())) {
      throw new ResourceNotFoundException(
          "Catalog with id [" + catalogId + "] not found for merchant [" + store.getCode() + "]");
    }
  ...
  @Override
  public ReadableCatalogList listCatalogs(
    ...
    try {
      ...
      throw new ServiceRuntimeException(
          "Cannot get catalogs for merchant [" + store.getCode() + "]");
    }
  ...
  @Override
  public ReadableCatalogEntryList listCatalogEntry(
    ...
    if (catalog == null) {
      throw new ResourceNotFoundException(
          "Catalog with id [" + id + "] not found for store [" + store.getCode() + "]");
    }

    ReadableCatalogEntryList catalogList = new ReadableCatalogEntryList();

    try {
      Page<CatalogEntry> entry =
      ...
      List<ReadableCatalogEntry> readableList =
          entry.getContent().stream()
      ...
      throw new ServiceRuntimeException(
          "Cannot get catalog entry for catalog ["
              + id
              + "] andr merchant ["
              + store.getCode()
              + "]");
  ...
  public ReadableCatalogEntry addCatalogEntry(
      PersistableCatalogEntry entry, MerchantStore store, Language language) {
