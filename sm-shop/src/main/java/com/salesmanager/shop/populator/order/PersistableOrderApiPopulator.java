...
public class PersistableOrderApiPopulator
    extends AbstractDataPopulator<PersistableOrderApi, Order> {
  ...
  public Order populate(
      PersistableOrderApi source, Order target, MerchantStore store, Language language)
      ...
      if (source.getCustomerId() != null && source.getCustomerId().longValue() > 0) {
        ...
        if (source instanceof PersistableAnonymousOrderApi) {
          PersistableCustomer persistableCustomer =
              ((PersistableAnonymousOrderApi) source).getCustomer();
