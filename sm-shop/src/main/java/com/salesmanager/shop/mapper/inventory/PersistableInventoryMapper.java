  ...
  public ProductAvailability convert(
    ...
    try {
      ...
      for (PersistableProductPrice priceEntity : source.getPrices()) {
        ...
        java.util.List<com.salesmanager.shop.model.catalog.product.ProductPriceDescription>
            descriptions = priceEntity.getDescriptions();
        if (descriptions != null) {
          Set<ProductPriceDescription> descs = new HashSet<ProductPriceDescription>();
          for (com.salesmanager.shop.model.catalog.product.ProductPriceDescription desc :
              descriptions) {
            ...
            if (!CollectionUtils.isEmpty(price.getDescriptions())) {
              for (ProductPriceDescription d : price.getDescriptions()) {
                if (desc.getId() != null
                    && desc.getId().longValue() > 0
                    && desc.getId().longValue() == d.getId().longValue()) {
                  desc.setId(d.getId());
                }
              }
            }
