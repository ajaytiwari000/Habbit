...
public class StoreFilter extends HandlerInterceptorAdapter {
  ...
  private static final String SERVICES_URL_PATTERN = "/services";
  private static final String REFERENCE_URL_PATTERN = "/reference";
      ...
      */
      ...
      if (customer == null) {
        ...
        if (auth != null && request.isUserInRole("AUTH_CUSTOMER")) {
          customer = customerService.getByUserName(auth.getName());
          if (customer != null) {
            request.setAttribute(Constants.CUSTOMER, customer);
          }
        }
      ...
      if (anonymousCustomer == null) {
        ...
        try {
          ...
          if (!StringUtils.isBlank(ipAddress)) {
            Addresss geoAddresss = customerService.getCustomerAddress(store, ipAddress);
            if (geoAddresss != null) {
              address = new Address();
              address.setCountry(geoAddresss.getCountry());
              address.setCity(geoAddresss.getCity());
              address.setZone(geoAddresss.getZone());
              /** no postal code * */
       ...
       *
       * <p>Elements from Header : - CMS links - Customer - Mini shopping cart - Store name / logo -
       * Top categories - Search
       *
       * <p>Elements from Footer : - CMS links - Store address - Global payment information - Global
  ...
  @SuppressWarnings("unchecked")
  private void getContentPageNames(
      MerchantStore store, Language language, HttpServletRequest request) throws Exception {
  ...
  private void setTopCategories(MerchantStore store, Language language, HttpServletRequest request)
    ...
    Map<String, List<ReadableCategorys>> objects = null;
    List<ReadableCategorys> loadedCategories = null;

    if (store.isUseCache()) {
      objects =
          (Map<String, List<ReadableCategorys>>)
              webApplicationCache.getFromCache(categoriesKey.toString());

      if (objects == null) {
        // load categories
        ReadableCategoryList categoryList =
            categoryFacade.getCategoryHierarchy(store, null, 0, language, null, 0, 200); // null
        loadedCategories = categoryList.getCategories();

        // filter out invisible category
        loadedCategories.stream()
            .filter(cat -> cat.isVisible() == true)
            .collect(Collectors.toList());

        objects = new ConcurrentHashMap<String, List<ReadableCategorys>>();
      ...
      ReadableCategoryList categoryList =
          categoryFacade.getCategoryHierarchy(
              store, null, 0, language, null, 0, 200); // null // filter
      loadedCategories = categoryList.getCategories();
    }
  ...
  private Map<String, List<ContentDescription>> getContentPagesNames(
  ...
  // Language language)
  // throws Exception {
  private Map<String, List<ReadableCategorys>> getCategories(MerchantStore store, Language language)
      throws Exception {

    // Map<String, List<Category>> objects = new ConcurrentHashMap<String,
    // List<Category>>();
    Map<String, List<ReadableCategorys>> objects =
        new ConcurrentHashMap<String, List<ReadableCategorys>>();
    ...
    ReadableCategorysPopulator readableCategorysPopulator = new ReadableCategorysPopulator();

    Map<String, ReadableCategorys> subs = new ConcurrentHashMap<String, ReadableCategorys>();
      ...
      for (Category category : categories) {
        if (category.isVisible()) {
          ...
          for (CategoryDescription description : descriptions) {
            ...
            readableCategorysPopulator.populate(category, readableCategorys, store, language);
            ...
            if (category.getDepth().intValue() == 0) {
              ...
              List<ReadableCategorys> cacheCategories = null;
              if (objects == null || objects.size() == 0) {
                // objects = new HashMap<String,
                // List<Category>>();
                objects = new HashMap<String, List<ReadableCategorys>>();
              }
              if (!objects.containsKey(key)) {
                // cacheCategories = new ArrayList<Category>();
                cacheCategories = new ArrayList<ReadableCategorys>();
              ...
              cacheCategories.add(readableCategorys);

            } else {
              subs.put(lang.getCode(), readableCategorys);
            }
          }
        }
      }
  ...
  private Map<String, Object> getConfigurations(MerchantStore store) {
    ...
    try {
      ...
      // get social
      List<MerchantConfiguration> socialConfigs =
          merchantConfigurationService.listByType(MerchantConfigurationType.SOCIAL, store);
      ...
      configs.put(
          Constants.FACEBOOK_APP_ID, coreConfiguration.getProperty(Constants.FACEBOOK_APP_ID));
  ...
  private MerchantStore setMerchantStoreInSession(HttpServletRequest request, String storeCode)
      throws Exception {
    if (storeCode == null || request == null) return null;
