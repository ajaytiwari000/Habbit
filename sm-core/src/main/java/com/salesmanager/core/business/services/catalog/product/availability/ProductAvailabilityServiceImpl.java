...
public class ProductAvailabilityServiceImpl
  ...
  public Page<ProductAvailability> listByProduct(
      Product product, MerchantStore store, String child, int page, int count)
      throws ServiceException {
    Validate.notNull(product, "Product cannot be null");
    Validate.notNull(store, "MercantStore cannot be null");
    Pageable pageRequest = PageRequest.of(page, count);
    return pageableProductAvailabilityRepository.listByStore(
        product.getId(), store.getId(), child, pageRequest);
  }
  ...
  @Override
  public ProductAvailability getByInventoryId(
      Long productId, Long availabilityId, MerchantStore store) throws ServiceException {
    Validate.notNull(store, "Merchant must not be null");
