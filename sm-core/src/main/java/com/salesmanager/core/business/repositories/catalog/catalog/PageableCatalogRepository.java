...
public interface PageableCatalogRepository extends PagingAndSortingRepository<Catalog, Long> {
  ...
  @Query(
      value =
          "select distinct c from Catalog c join fetch c.merchantStore cm where cm.id=?1 and (?2 is null or c.code like %?2%)",
      countQuery =
          "select count(c) from Catalog c join c.merchantStore cm where cm.id=?1 and (?2 is null or c.code like %?2%)")
