...
public class UserServiceImpl extends SalesManagerEntityServiceImpl<Long, User>
  ...
  @Autowired private PageableUserRepository pageableUserRepository;
  ...
  public Page<User> listByCriteria(UserCriteria criteria, int page, int count)
    ...
    if (criteria.getStoreIds() != null) { // search within a predefined list of stores
      users =
          pageableUserRepository.listByStoreIds(
              criteria.getStoreIds(), criteria.getAdminEmail(), pageRequest);
      ...
      users =
          pageableUserRepository.listByStore(
              criteria.getStoreCode(), criteria.getAdminEmail(), pageRequest);
    }
