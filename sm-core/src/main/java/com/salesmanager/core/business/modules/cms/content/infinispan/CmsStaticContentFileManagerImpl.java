...
public class CmsStaticContentFileManagerImpl
   ...
   * </pre>
   *
   * <p>Merchant store code will be used to create cache node where merchant data will be
   * stored,input data will contain name, file as well type of data being stored.
   *
   * @see FileContentType
   ...
   * @param inputStaticContentDataList input static content file list which will get {@link
   *     InputContentImage} stored
  ...
  public void addFiles(
      final String merchantStoreCode, final List<InputContentFile> inputStaticContentDataList)
      throws ServiceException {
    if (cacheManager.getTreeCache() == null) {
      LOGGER.error("Unable to find cacheManager.getTreeCache() in Infinispan..");
      throw new ServiceException(
  ...
  public OutputContentFile getFile(
      ...
      final FileContentType fileContentType,
    ...
    try {
      ...
      outputStaticContentData.setMimeType(
          URLConnection.getFileNameMap().getContentTypeFor(contentFileName));
  ...
  public List<OutputContentFile> getFiles(
      final String merchantStoreCode, final FileContentType staticContentType)
      throws ServiceException {
