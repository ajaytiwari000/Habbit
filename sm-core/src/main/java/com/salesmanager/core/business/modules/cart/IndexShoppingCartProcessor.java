...
@Component
public class IndexShoppingCartProcessor extends IndexEntityProcessor
    implements ShoppingCartProcessor {
  ...
  public void process(String event, Object entity, Customer customer, MerchantStore store) {
    ...
    try {
      ...
      String indexName =
          new StringBuilder().append(INDEX_NAME).append(store.getCode().toLowerCase()).toString();
      ...
      if (response.getResult() != DocWriteResponse.Result.CREATED
        ...
        LOGGER.error(
            "An error occured while indexing an shopping cart document "
                + cart.getId()
                + " "
