...
public class DefaultPackagingImpl implements Packaging {
  ...
  /** default dimensions * */
  private static final Double defaultWeight = 1D;

  private static final Double defaultHeight = 4D;
  private static final Double defaultLength = 4D;
  private static final Double defaultWidth = 4D;
  ...
  public List<PackageDetails> getBoxPackagesDetails(
    ...
    for (Product p : individualProducts) {
      ...
      double productVolume =
          (p.getProductWidth().doubleValue()
              * p.getProductHeight().doubleValue()
