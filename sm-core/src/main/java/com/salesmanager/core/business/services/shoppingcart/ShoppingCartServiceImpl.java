...
public class ShoppingCartServiceImpl extends SalesManagerEntityServiceImpl<Long, ShoppingCart>
  ...
  @Inject private ShoppingCartItemRepository shoppingCartItemRepository;

  @Inject private ShoppingCartAttributeRepository shoppingCartAttributeItemRepository;
  ...
  private void getPopulatedItem(final ShoppingCartItem item) throws Exception {
    ...
    List<ProductAttribute> attributesList =
        new ArrayList<ProductAttribute>(); // attributes maintained
    List<ShoppingCartAttributeItem> removeAttributesList =
        new ArrayList<ShoppingCartAttributeItem>(); // attributes to remove
    ...
    BigDecimal subTotal =
        item.getItemPrice().multiply(new BigDecimal(item.getQuantity().intValue()));
  ...
  public ShoppingCart mergeShoppingCarts(
    ...
    if (sessionCart.getCustomerId() != null
        && sessionCart.getCustomerId() == userShoppingModel.getCustomerId()) {
    ...
    if (CollectionUtils.isNotEmpty(sessionCart.getLineItems())) {
      ...
      for (ShoppingCartItem shoppingCartItem : sessionCart.getLineItems()) {
        ...
        List<ShoppingCartAttributeItem> cartAttributes =
            new ArrayList<ShoppingCartAttributeItem>(shoppingCartItem.getAttributes());
        if (CollectionUtils.isNotEmpty(cartAttributes)) {
          for (ShoppingCartAttributeItem shoppingCartAttributeItem : cartAttributes) {
            ProductAttribute productAttribute =
                productAttributeService.getById(shoppingCartAttributeItem.getId());
    ...
    if (item != null) {
      ...
      if (item.getAttributes() != null) {
        item.getAttributes().stream()
            .forEach(
                a -> {
                  shoppingCartAttributeItemRepository.deleteById(a.getId());
                });
