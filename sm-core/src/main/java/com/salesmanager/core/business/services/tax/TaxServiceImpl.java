...
public class TaxServiceImpl implements TaxService {
  ...
  private static final String TAX_CONFIGURATION = "TAX_CONFIG";
  private static final String DEFAULT_TAX_CLASS = "DEFAULT";
  ...
  public TaxConfiguration getTaxConfiguration(MerchantStore store) throws ServiceException {
    ...
    MerchantConfiguration configuration =
        merchantConfigurationService.getMerchantConfiguration(TAX_CONFIGURATION, store);
  ...
  @Override
  public List<TaxItem> calculateTax(
    ...
    Country country = null; // customer.getBilling().getCountry();
    Zone zone = null; // customer.getBilling().getZone();
    String stateProvince = null; // customer.getBilling().getState();
    ...
    if (taxBasisCalculation.name().equals(TaxBasisCalculation.SHIPPINGADDRESS)) {
      Delivery shipping = null; // customer.getDelivery();
      if (shipping != null) {
      ...
      Billing billing = null; // customer.getBilling();
      if (billing != null) {
    ...
    if (shippingSummary != null
        && shippingSummary.getShipping() != null
    ...
    for (Long taxClassId : taxClassAmountMap.keySet()) {
      ...
      if (!StringUtils.isBlank(stateProvince) && zone == null) {
        taxRates =
            taxRateService.listByCountryStateProvinceAndTaxClass(
                country, stateProvince, taxClasses.get(taxClassId), store, language);
      } else {
        taxRates =
            taxRateService.listByCountryZoneAndTaxClass(
                country, zone, taxClasses.get(taxClassId), store, language);
      }
