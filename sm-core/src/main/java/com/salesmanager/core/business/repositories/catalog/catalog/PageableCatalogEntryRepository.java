...
public interface PageableCatalogEntryRepository
    extends PagingAndSortingRepository<CatalogEntry, Long> {

  @Query(
      value =
          "select distinct c from CatalogEntry c join fetch c.product cp "
      ...
      countQuery =
          "select  count(c) from CatalogEntry c join c.product cp join c.category cc join c.catalog cl join cl.merchantStore clm join cp.descriptions cpd where cl.id=?1 and clm.id=?2 and cpd.language.id=?3 and (?4 is null or cpd.name like %?4%)")
  Page<CatalogEntry> listByCatalog(
