...
public class BeanStreamPayment implements PaymentModule {
  ...
  public Transaction authorize(
    ...
    return processTransaction(
        store, customer, TransactionType.AUTHORIZE, amount, payment, configuration, module);
  }
  ...
  public Transaction capture(
    ...
    try {
       ...
       * &trnType=PAC&username=user1234&password=pass1234&trnID=1000 2115 --> requires also adjId
       * [not documented]
       */
      ...
      messageString
          .append("merchant_id=")
          .append(configuration.getIntegrationKeys().get("merchantid"))
          .append("&");
      messageString.append("trnType=").append("PAC").append("&");
      messageString
          .append("username=")
          .append(configuration.getIntegrationKeys().get("username"))
          .append("&");
      messageString
          .append("password=")
          .append(configuration.getIntegrationKeys().get("password"))
          .append("&");
  ...
  public Transaction authorizeAndCapture(
      ...
      throws IntegrationException {
    return processTransaction(
        store, customer, TransactionType.AUTHORIZECAPTURE, amount, payment, configuration, module);
  }
  ...
  public Transaction refund(
    ...
    try {
      ...
      server =
          new StringBuffer()
              .append(configs.getScheme())
              .append("://")
              .append(configs.getHost())
              .append(":")
              .append(configs.getPort())
              .append(configs.getUri())
              .toString();
      ...
      /**
       * merchant_id=123456789&requestType=BACKEND &trnType=R&username=user1234&password=pass1234
      ...
      messageString
          ...
          .append("&");
      messageString
          .append("trnOrderNumber=")
          .append(transaction.getTransactionDetails().get("TRNORDERNUMBER"))
          .append("&");
      messageString.append("trnAmount=").append(amnt).append("&");
  ...
  private Transaction sendTransaction(
    ...
    try {
      ...
      /**
       * requestType=BACKEND&merchant_id=300200260 &trnType=P &username=carlito&password=shopizer001
       * &orderNumber=caa71106-7e3f-4975-a657-a35904dc32a0 &trnCardOwner=Carl Samson
       * &trnCardNumber=5100000020002000 &trnExpMonth=10 &trnExpYear=14 &trnCardCvd=123
       * &trnAmount=77.01 &ordName=Carl S &ordAddress1=378 Du Languedoc &ordCity=Boucherville
       * &ordProvince=QC &ordPostalCode=J4B8J9 &ordCountry=CA &ordPhoneNumber=(444) 555-6666
       ...
       * &trnCardNumber=4030000010001234 &trnExpMonth=10 &trnExpYear=16 &ordName=Joe+Test
       * &ordAddress1=123+Test+Street &ordCity=Victoria &ordProvince=BC &ordCountry=CA
       * &ordPostalCode=V8T2E7 &ordPhoneNumber=5555555555 &ordEmailAddress=joe%40testemail.com
       */
      boolean bSandbox = false;
      if (configuration.getEnvironment().equals("TEST")) { // sandbox
        bSandbox = true;
      ...
      if (transactionApproved.equals("0")) {
        ...
        IntegrationException te =
            new IntegrationException("Can't process BeanStream message " + messageText);
  ...
  private Transaction processTransaction(
    ...
    try {
      ...
      messageString
          .append("trnCardNumber=")
          .append(creditCardPayment.getCreditCardNumber())
          .append("&");
      messageString
          .append("trnExpMonth=")
          .append(creditCardPayment.getExpirationMonth())
          .append("&");
      messageString
          .append("trnExpYear=")
          .append(creditCardPayment.getExpirationYear().substring(2))
          .append("&");
      messageString
          .append("trnCardCvd=")
          .append(creditCardPayment.getCredidCardValidationNumber())
          .append("&");
      messageString.append("trnAmount=").append(amnt).append("&");

      StringBuilder nm = new StringBuilder();
      //      nm.append(customer.getBilling().getFirstName())
      //          .append(" ")
      ...
      //      messageString
      //          .append("ordPostalCode=")
      //          .append(customer.getBilling().getPostalCode().replaceAll("\\s", ""))
      //          .append("&");
      //      messageString.append("ordCountry=").append(countryName).append("&");
      //      messageString
      //          .append("ordPhoneNumber=")
      //          .append(customer.getBilling().getTelephone())
      //          .append("&");
       ...
       *
       * <p>pre authorization (PA) ----------------------
       *
       * <p>Prior to processing a pre-authorization through the API, you must modify the transaction
       * settings in your Beanstream merchant member area to allow for this transaction type. - Log
       * in to the Beanstream online member area at www.beanstream.com/admin/sDefault.asp. -
       * Navigate to administration - account admin - order settings in the left menu. Under the
       * heading �Restrict Internet Transaction Processing Types,� select either of the last two
       * options. The �Purchases or Pre-Authorization Only� option will allow you to process both
       * types of transaction through your web interface. De-selecting the �Restrict Internet
       * Transaction Processing Types� checkbox will allow you to process all types of transactions
       * including returns, voids and pre-auth completions.
       *
       * <p>capture (PAC) -> requires trnId -------------
       *
       * <p>refund (R) ------------- REQUEST ->
       ...
       *
       * <p>//notes //On receipt of the transaction response, the merchant must display order
       * amount, transaction ID number, bank authorization code (authCode), currency, date and
      ...
      messageLogString
          .append("trnExpYear=")
          .append(creditCardPayment.getExpirationYear())
          .append("&");
      messageLogString
          .append("trnCardCvd=")
          .append(creditCardPayment.getCredidCardValidationNumber())
      ...
      messageLogString
          .append("ordAddress1=")
          //          .append(customer.getBilling().getAddress())
          //          .append("&");
          //      messageLogString.append("ordCity=").append(customer.getBilling().getCity())
          .append("&");

      //      messageLogString.append("ordProvince=").append(stateProvince).append("&");
      //      messageLogString
      //          .append("ordPostalCode=")
      //          .append(customer.getBilling().getPostalCode())
      //          .append("&");
      //      messageLogString
      //          .append("ordCountry=")
      //          .append(customer.getBilling().getCountry().getName())
      //          .append("&");
      //      messageLogString
      //          .append("ordPhoneNumber=")
      //          .append(customer.getBilling().getTelephone())
  ...
  private Transaction parseResponse(
      TransactionType transactionType,
      PaymentType paymentType,
