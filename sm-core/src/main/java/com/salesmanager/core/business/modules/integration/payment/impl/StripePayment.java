...
public class StripePayment implements PaymentModule {
    ...
    try {
      ...
      if (payment.getPaymentMetaData() == null || StringUtils.isBlank(apiKey)) {
        IntegrationException te =
            new IntegrationException("Can't process Stripe, missing payment.metaData");
      ...
      if (StringUtils.isBlank(token)) {
        IntegrationException te =
            new IntegrationException("Can't process Stripe, missing stripe token");
        te.setExceptionType(IntegrationException.TRANSACTION_EXCEPTION);
        te.setMessageCode("message.payment.error");
        te.setErrorCode(IntegrationException.TRANSACTION_EXCEPTION);
        throw te;
  ...
  public Transaction capture(
    ...
    try {
      ...
      if (StringUtils.isBlank(chargeId)) {
        IntegrationException te =
            new IntegrationException("Can't process Stripe capture, missing TRNORDERNUMBER");
        te.setExceptionType(IntegrationException.TRANSACTION_EXCEPTION);
        te.setMessageCode("message.payment.error");
        te.setErrorCode(IntegrationException.TRANSACTION_EXCEPTION);
        throw te;
  ...
  private IntegrationException buildException(Exception ex) {
    ...
    if (ex instanceof CardException) {
      ...
      /**
       * invalid_number The card number is not a valid credit card number. invalid_expiry_month The
       * card's expiration month is invalid. invalid_expiry_year The card's expiration year is
       * invalid. invalid_cvc The card's security code is invalid. incorrect_number The card number
       * is incorrect. expired_card The card has expired. incorrect_cvc The card's security code is
       * incorrect. incorrect_zip The card's zip code failed validation. card_declined The card was
       * declined. missing There is no card on a customer that is being charged. processing_error An
       * error occurred while processing the card. rate_limit An error occurred due to requests
       * hitting the API too quickly. Please let us know if you're consistently running into this
       * error.
       */
      ...
      if ("card_declined".equals(declineCode)) {
        IntegrationException te =
            new IntegrationException("Can't process stripe message " + e.getMessage());
      ...
      IntegrationException te =
          new IntegrationException("Can't process stripe card  " + e.getMessage());
      te.setExceptionType(IntegrationException.EXCEPTION_VALIDATION);
      te.setMessageCode("messages.error.creditcard.number");
      te.setErrorCode(IntegrationException.EXCEPTION_VALIDATION);
      return te;
      ...
      IntegrationException te =
          new IntegrationException("Can't process Stripe, missing invalid payment parameters");
      te.setExceptionType(IntegrationException.TRANSACTION_EXCEPTION);
      te.setMessageCode("messages.error.creditcard.number");
      te.setErrorCode(IntegrationException.TRANSACTION_EXCEPTION);
      return te;
      ...
      if (ex instanceof IntegrationException) {
        ...
        IntegrationException te =
            new IntegrationException("Can't process Stripe authorize, exception", ex);
        te.setExceptionType(IntegrationException.TRANSACTION_EXCEPTION);
        te.setMessageCode("message.payment.error");
        te.setErrorCode(IntegrationException.TRANSACTION_EXCEPTION);
        return te;
