...
/**
 * Uses google api to get lng, lat and distance in km for a given delivery address The route will be
 * displayed on a map to the end user and available from the admin section
 *
 * <p>The module can be configured to use miles by changing distance.inMeters
 *
 * <p>To use this pre-processor you will need a google api-key
 *
 * <p>Access google developers console https://console.developers.google.com/project
 *
 * <p>Geocoding Distance Matrix Directions
 *
 * <p>Create new key for server application Copy API key https://console.developers.google.com
 *
 * <p>https://developers.google.com/maps/documentation/webservices/client-library
...
public class ShippingDistancePreProcessorImpl implements ShippingQuotePrePostProcessModule {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(ShippingDistancePreProcessorImpl.class);
  ...
  public void prePostProcessShippingQuotes(
    ...
    originAddress
        .append(origin.getAddress())
        .append(BLANK)
        .append(origin.getCity())
        .append(BLANK)
        .append(origin.getPostalCode())
        .append(BLANK);
    ...
    try {
      GeocodingResult[] originAdressResult =
          GeocodingApi.geocode(context, originAddress.toString()).await();

      GeocodingResult[] destinationAdressResult =
          GeocodingApi.geocode(context, destinationAddress.toString()).await();
      ...
      if (originAdressResult.length > 0 && destinationAdressResult.length > 0) {
        ...
        DistanceMatrix distanceRequest =
