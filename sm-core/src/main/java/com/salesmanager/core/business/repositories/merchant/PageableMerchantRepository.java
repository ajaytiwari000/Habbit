...
public interface PageableMerchantRepository
  ...
  @Query(
      value =
          "select distinct m from MerchantStore m left join fetch m.parent mp left join fetch m.country mc left join fetch m.currency mc left join fetch m.zone mz left join fetch m.defaultLanguage md left join fetch m.languages mls where mp.code = ?1",
      countQuery =
          "select count(distinct m) from MerchantStore m join m.parent mp where mp.code = ?1")
  Page<MerchantStore> listByStore(String code, Pageable pageable);

  @Query(
      value =
          "select distinct m from MerchantStore m left join fetch m.parent mp left join fetch m.country mc left join fetch m.currency mc left join fetch m.zone mz left join fetch m.defaultLanguage md left join fetch m.languages mls where (?1 is null or m.storename like %?1%)",
      countQuery =
          "select count(distinct m) from MerchantStore m where (?1 is null or m.storename like %?1%)")
  ...
  @Query(
      value =
          "select distinct m from MerchantStore m left join m.parent mp "
  ...
  /**
   * @Query(value = "select distinct p from MerchantStore p left join fetch p.stores ps " + "with
   * ps.parent.code = p.code " + "where p.code = ?1 or ps.code = ?1 " + "and (?2 is null or
   * (p.storename like %?2% or ps.storename like %?2%))", countQuery = "select count(distinct p)
   * from MerchantStore p left join p.stores ps " + "with ps.parent.code = p.code where p.code = ?1
   * or ps.code = ?1 and (?2 is null or (p.storename like %?2% or ps.storename like %?2%))")
  ...
  */
  @Query(
      value =
          "select * from MERCHANT_STORE m "
              + "where (m.STORE_CODE = ?1 or (?2 is null or m.PARENT_ID = ?2)) "
              + "and (?3 is null or m.STORE_NAME like %?3%)",
      countQuery =
          "select count(*) from MERCHANT_STORE m where (m.STORE_CODE = ?1 or (?2 is null or m.PARENT_ID = ?2)) and (?3 is null or m.STORE_NAME like %?3%)",
      nativeQuery = true)
