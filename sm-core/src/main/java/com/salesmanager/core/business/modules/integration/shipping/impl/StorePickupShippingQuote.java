 ...
 *
 * <p>Requires a configuration of a message note to be printed to the client and a price for
 * calculation (should be configured to 0)
 *
 * <p>Calculates a ShippingQuote with a price set to the price configured
 *
 * @author carlsamson
 */
public class StorePickupShippingQuote
    implements ShippingQuoteModule, ShippingQuotePrePostProcessModule {

  public static final String MODULE_CODE = "storePickUp";
  ...
  public void prePostProcessShippingQuotes(
    ...
    Validate.notNull(
        globalShippingConfiguration, "IntegrationConfiguration must not be null for StorePickUp");

    try {

      if (!globalShippingConfiguration.isActive()) return;
      ...
      shippingOption.setOptionPriceText(
          productPriceUtils.getStoreFormatedAmountWithCurrency(
              store, productPriceUtils.getAmount(price)));
