...
public class MerchantStoreServiceImpl extends SalesManagerEntityServiceImpl<Integer, MerchantStore>
  ...
  public MerchantStore getParent(String code) throws ServiceException {
    ...
    if (storeModel.isRetailer() != null
        && storeModel.isRetailer()
        && storeModel.getParent() == null) {
      return storeModel;
    }
  ...
  /**
   * Store might be alone (known as retailer) A retailer can have multiple child attached
   *
   * <p>This method from a store code is able to retrieve parent and childs. Method can also filter
   * on storeName
   */
  @Override
