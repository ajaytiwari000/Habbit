...
public class OrderServiceImpl extends SalesManagerEntityServiceImpl<Long, Order>
  ...
  private OrderTotalSummary caculateOrder(
      OrderSummary summary, Customer customer, final MerchantStore store, final Language language)
      throws Exception {
    ...
    if (OrderSummaryType.ORDERTOTAL.name().equals(summary.getOrderSummaryType().name())
      ...
      // Post processing order total variation modules for sub total calculation - drools, custom
      // modules
  ...
  private OrderTotalSummary caculateShoppingCart(
    ...
    itemList =
        itemList.stream().filter(p -> p.getProduct().isAvailable()).collect(Collectors.toList());
  ...
  /**
   * Method will be used to calculate Shopping cart total as well will update price for each line
   * items.
   *
   * @param shoppingCart
   * @param customer
  ...
  @Override
  public ByteArrayOutputStream generateInvoice(
      final MerchantStore store, final Order order, final Language language)
      throws ServiceException {
  ...
  public List<Order> getCapturableOrders(MerchantStore store, Date startDate, Date endDate)
    ...
    if (!CollectionUtils.isEmpty(transactions)) {
      ...
      /** Order id Transaction type 1 AUTHORIZE 1 CAPTURE */

      // should have when not captured
      /** Order id Transaction type 2 AUTHORIZE */
