...
public class SESEmailSenderImpl implements EmailModule {
  ...
  @Inject private Configuration freemarkerMailConfiguration;
            ...
            .withMessage(
                new Message()
                    .withBody(
                        new Body()
                            .withHtml(
                                new Content().withCharset("UTF-8").withData(prepareHtml(email)))
  ...
  private String prepareHtml(Email email) throws Exception {
    ...
    Template htmlTemplate =
        freemarkerMailConfiguration.getTemplate(
            new StringBuilder(TEMPLATE_PATH)
                .append("")
                .append("/")
                .append(email.getTemplateName())
                .toString());
